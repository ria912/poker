        ...
        <Paper elevation={3} sx={{ p: 2, bgcolor: 'rgba(0,0,0,0.5)', color: 'white' }}>
            ...
            {betOrRaiseInfo && (
                <Box sx={{ mt: 2 }}>
                    ...
                    <Slider
                        value={raiseAmount}
                        min={betOrRaiseInfo.min}
                        max={betOrRaiseInfo.max}
                        step={10}
                        onChange={(_, newValue) => setRaiseAmount(newValue)}
                        valueLabelDisplay="auto"
                    />
                    <Button
                        variant="contained"
                        color="primary"
                        onClick={() => onAction({ type: betOrRaiseInfo.type, amount: raiseAmount })}
                        disabled={loading || raiseAmount < betOrRaiseInfo.min || raiseAmount > betOrRaiseInfo.max}
                        fullWidth
                    >
